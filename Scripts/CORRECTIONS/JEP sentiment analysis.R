...
p1 <- tidy_subtitles_annotated %>%
  ...
  ggplot(aes(x = Token, y = n, fill = n)) +
  geom_col(alpha = 1) +
  scale_fill_gradient(low = color_inicial, high = color_final) +
  xlab(NULL) +
  ylab("Frecuencia") +
  coord_flip() +
...
tidy_subtitles_annotated %>%
  ...
  ggplot(aes(x = reorder(lemma, n), y = n, fill = n)) +
  geom_col(alpha = 1) +
  scale_fill_gradient(low = color_inicial, high = color_final) +
  labs(x = "Lema", y = "Frecuencia", title = "") +
  coord_flip() +
  theme_light(base_size = 13) +
  theme(
...
png("serie_score_Correction.png",
    width = 12, height = 8, units = "cm", res = 300, pointsize = 8, bg = "white"
)

ggplot(data, aes(x = x)) +
  geom_line(aes(y = MN, color = "Negativo"), linewidth = 0.5) +
  geom_line(aes(y = MP, color = "Positivo"), linewidth = 0.5) +
  labs(
    x = "Índice",
    y = "Puntaje medio de sentimiento",
    title = ""
  ) +
  scale_color_manual(
    name = NULL,
    ...
    values = c(
      "Negativo" = color_final,      # color oscuro
      "Positivo" = color_inicial     # color claro
    )
  ) +
  theme_light(base_size = 13) +
  theme(
    ...
    panel.grid.minor = element_line(size = 0.2, colour = "grey90"),
    axis.line = element_line(linewidth = 0.4),
    panel.border = element_rect(fill = NA, linewidth = 0.4)
  )
dev.off()
