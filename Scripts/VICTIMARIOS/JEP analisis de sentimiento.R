...
setwd("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Analisis-Sentimientos - Victimarios/General")
...
p1 <- tidy_subtitles_annotated  %>% filter(Token!='él',Token!='yo')%>%
  count(Token, sort = TRUE) %>%
  filter(n > 20630) %>%
...
ggsave("frec.png",
       path = "C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Analisis-Sentimientos - Victimarios/General", 
       plot = p1, 
...
png("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Analisis-Sentimientos - Victimarios/General/word_cloud.png",
    width = 12, height = 8, units = "cm", res = 300, pointsize = 8, bg = "white")
set.seed(12)
tidy_subtitles_annotated %>%
  count(Token, sort = TRUE) %>% filter(Token!='él',Token!='yo',Token!='tú')%>%
  with(wordcloud(words = Token, scale = c(3, 0.4), freq = n, max.words = 200, colors = Col))
dev.off()
...
tidy_subtitles_annotated %>% 
  ...
  labs(x = "Etiqueta POS", y = "Frequency", title = "Subtítulos: Video JEP") +
  coord_flip() +
  theme(legend.position = "none", text = element_text(size = 18))

# Análisis de los 5 lemas más frecuentes por categoría gramatical (NOUN, PROPN, VERB, ADJ)
png("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Analisis-Sentimientos - Victimarios/General/noun.png",
    width = 12, height = 8, units = "cm", res = 300, pointsize = 8, bg = "white")
tidy_subtitles_annotated %>% 
...
png("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Analisis-Sentimientos - Victimarios/General/verb.png", 
    width = 12, height = 8, units = "cm", res = 300, pointsize = 8, bg = "white")
...
AFIN_completo <- AFIN_negativo %>%
  ...
  replace_na(list(media_negativo = 0, media_positivo = 0, frecuencia_negativo = 0, frecuencia_positivo = 0))
...
hist(AFIN_completo$media_positivo, main = "",
...
hist((-1)*AFIN_completo$media_negativo, prob = TRUE, col = rgb(1, 0.2, 0.2, 0.5),
