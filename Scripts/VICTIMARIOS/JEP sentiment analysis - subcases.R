...
library(xtable)
library(readxl)
# Read automatic subtitles from the list of case 03 of the JEP from YouTube
...
SubCaso_CC <- read_csv("C:/Users/Pc/Desktop/-Proyecto-JEP-/Data/CostaCaribe_subtitles.csv") # Subtitles for Caribbean Coast
...
# META, ANTIOQUIA AND NORTH OF SANTANDER DO NOT HAVE SUFFICIENT VIDEOS
...
setwd(paste0("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Sentiment Analysis - perpetrators/", Palabra))
...
if (Palabra == 'Norte de santander') {
  subtitulos_JEP_03 <- subtitulos_JEP_03 %>%
    ...
    filter(str_detect(Titulo, Palabra) | str_detect(Titulo, 'Catatumbo')) %>% # Only for North of Santander
    as.data.frame()
} else {
  # Other actions if the word is not "North of Santander"
  subtitulos_JEP_03 <- subtitulos_JEP_03 %>%
    filter(Titulo %in% Todos_Titles) %>%
    filter(!(Titulo %in% SubCaso$Titulo) & !is.na(Subtitulos)) %>% # Filter titles that are NOT in SubCaso
    filter(str_detect(Titulo, Palabra)) %>% # Filter those that contain the word
...
# A dataframe is created with titles that may contain keywords
...
subtitulos_JEP_03 <- within(subtitulos_JEP_03, {
  ...
  Subtitulos <- str_squish(Subtitulos) # Remove extra white spaces
})
