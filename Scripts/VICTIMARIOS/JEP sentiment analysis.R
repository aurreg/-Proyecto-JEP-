# Libraries
# lemmatizer udpipe
library(udpipe)     # udpipe lemmatizer
library(stringr)     # string manipulation
library(tibble)      # tibbles (data frames)
...
library(ngram)       # n-gram analysis
library(xtable)      # LaTeX table creation
library(readxl)       # Read excel files
# Read automatic subtitles from the JEP case 03 YouTube list
setwd("C:/Users/Pc/Desktop/-Proyecto-JEP-/Outputs/Sentiment Analysis - perpetrators/General")
...
tidy_subtitles_annotated %>%
  ...
  with(wordcloud(words = Token, scale = c(3, 0.4), freq = n, max.words = 175, colors = Col))
dev.off()
